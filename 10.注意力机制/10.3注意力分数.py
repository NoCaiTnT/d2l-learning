...
#       a(q,k_i) = <q,k_i> / sqrt(d)   (1×d) * (d×1) = 1
#   为什么要除以sqrt(d)?
#       当维度很大时, 内积乘出来的数很大, 除以sqrt(d)是为了缩放点积, 防止内积过大。
#       如果不对这些点积进行缩放，输入到softmax函数的值可能会变得非常大，从而导致softmax函数的输出接近于0或1。这样会使得梯度在反向传播时变得非常小或不稳定，导致训练困难。
#   为什么是除以根号d而不是其他?
#       对于长度为d的向量它们的点积可以被视为d个独立的随机变量的和, 这些随机变量的和趋向于正态分布, 且其方差与d成正比, 为了使得点积的方差不随着d的增大而增大, 所以要除以根号d
#   向量化版本:
