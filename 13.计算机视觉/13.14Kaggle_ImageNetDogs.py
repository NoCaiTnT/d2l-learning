...
# 整理数据集, 将每类的图片放到一个文件夹下, 验证集为10%, 放到train_valid_test中
...
with open('13.14submission.csv', 'w') as f:
...
# 与13.13不一样的地方
#   图像增广不一样, 裁剪上可以做文章
#   数据集相对复杂, 在颜色上做了变化, 用了resnet34, 并用了微调, 并进行了冻结
#   在预测时, 不止预测单一的, 而是每一类都进行了预测, 给出概率
