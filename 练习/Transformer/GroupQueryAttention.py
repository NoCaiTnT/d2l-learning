...
class GroupQueryAttention(nn.Module):
    def __init__(self, num_hiddens, num_heads, num_groups, query_size, key_size, value_size, dropout):
    ...
    def forward(self, q, k, v, valid_len=None):
        ...
        if valid_len is not None:
            valid_len = torch.tensor([[i] * self.num_heads for i in valid_len], device=q.device).reshape(-1)
    ...
    def share(self, X):
        X = X.unsqueeze(1)
        X = X.expand(-1, self.num_groups, -1, -1)
        X = X.reshape(-1, X.size(2), X.size(3))
        X = X.repeat(1, 1, self.num_heads // self.num_groups)
        return X

    def split(self, X):
        X = X.reshape(X.size(0), X.size(1), self.num_groups, -1)
        X = X.permute(0, 2, 1, 3)
        X = X.reshape(-1, X.size(2), X.size(3))
        return X

    def merge(self, X):
        X = X.reshape(-1, self.num_groups, X.size(1), X.size(2))
        X = X.permute(0, 2, 1, 3)
...
attention = GroupQueryAttention(num_hiddens, num_heads, num_groups, query_size, key_size, value_size, dropout)
